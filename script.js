// this could be done a lot better and will be improved
const html = `1 It is wise for those who hear, not me, but the universal Reason, to confess that all things are one.
...
// Split the HTML file into an array of strings, using the numbers as the delimiter
const fragments = html.split(/\d+/);

const fragmentsArray = [];

// Loop through the sentences and add them to the array
fragments.forEach(fragment => fragmentsArray.push(fragment.trim()));

// Print the array of sentences
// console.log(sentencesArray);

// Generate a random index between 0 and the length of the sentencesArray
const randomIndex = Math.floor(Math.random() * fragmentsArray.length);

// Use the random index to retrieve a random sentence from the sentencesArray
const randomFragment = fragmentsArray[randomIndex];
